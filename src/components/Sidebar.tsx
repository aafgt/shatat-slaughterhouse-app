    ...
    return (
        <aside className="bg-green-950 w-2/12 flex flex-col justify-between h-[calc(100vh-4rem)] py-5 sticky top-16">
            <div className="flex flex-col gap-5">
                <NavLink to="/shatat-slaughterhouse/dashboard" className={({ isActive }) => `
                    py-1.5 px-5 flex items-center gap-2 max-[57rem]:justify-center hover:underline ${isActive ? "bg-green-900 rounded-l-lg font-bold" : undefined}
                `}>
                    <DashboardIcon /> <span className="max-[57rem]:hidden">Dashboard</span>
                </NavLink>

                <NavLink to="/shatat-slaughterhouse/report" className={({ isActive }) => `
                    py-1.5 px-5 flex items-center gap-2 max-[57rem]:justify-center hover:underline ${isActive ? "bg-green-900 rounded-l-lg font-bold" : undefined}
                `}>
                    <DashboardIcon /> <span className="max-[57rem]:hidden">Report</span>
                </NavLink>

                <NavLink to="/shatat-slaughterhouse/receipt" className={({ isActive }) => `
                    py-1.5 px-5 flex items-center gap-2 max-[57rem]:justify-center hover:underline ${isActive ? "bg-green-900 rounded-l-lg font-bold" : undefined}
                `}>
                    <DashboardIcon /> <span className="max-[57rem]:hidden">Receipt</span>
                </NavLink>

                <NavLink to="/shatat-slaughterhouse/clients" className={({ isActive }) => `
                    py-1.5 px-5 flex items-center gap-2 max-[57rem]:justify-center hover:underline ${isActive ? "bg-green-900 rounded-l-lg font-bold" : undefined}
                `}>
